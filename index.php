...
$kirbySetup = __DIR__ . '/kirby/config/setup.php';
...
if (!is_file($kirbySetup)) {
    ...
    echo '<p>Der Ordner <code>kirby/</code> fehlt oder ist unvollständig. Dadurch entsteht der Fehler ';
    echo '<code>Failed opening required "kirby/config/setup.php"</code>.</p>';
    echo '<ol>';
    echo '<li>Falls noch Reste in <code>vendor/</code> liegen, lösche den Ordner komplett.</li>';
    echo '<li>Installiere die Abhängigkeiten neu mit <code>composer install</code> oder kopiere den kompletten Kirby-Ordner manuell nach <code>kirby/</code>.</li>';
    echo '<li>Stelle sicher, dass <code>kirby/config/setup.php</code> danach existiert.</li>';
    echo '</ol>';
    echo '</body></html>';
    return;
}
...
if (!is_file($autoload) || !is_file($kirbySetup)) {
    ...
    echo '<p>Die Anwendung benötigt die Kirby-Core-Dateien unter <code>kirby/</code> und den Composer-Autoloader unter <code>vendor/autoload.php</code>. ';
    ...
    echo '<li>Ohne Internet/Proxy: Lade das Kirby-Paket manuell von <a href="https://github.com/getkirby/cms/releases">github.com/getkirby/cms/releases</a> herunter und entpacke den <code>kirby</code>-Ordner in das Projektverzeichnis (Pfad muss <code>kirby/config/setup.php</code> enthalten).</li>';
...
if (!class_exists(Kirby::class)) {
    ...
    echo 'h1{font-size:1.6rem;margin-bottom:0.75rem;}code{background:#f5f5f5;padding:0.15rem 0.35rem;border-radius:4px;}ol{padding-left:1.1rem;}li{margin:0.35rem 0;}';
    ...
    echo '<p class="box">Der Autoloader wurde gefunden, aber die Kirby-Klassen fehlen.</p>';
    echo '<ol>';
    echo '<li><strong>Empfohlen:</strong> <code>composer install</code> im Projektstamm ausführen.</li>';
    echo '<li><strong>Offline:</strong> Kirby-Release von <a href="https://github.com/getkirby/cms/releases">github.com/getkirby/cms/releases</a> laden und den Ordner <code>kirby</code> ins Projekt kopieren.</li>';
    echo '<li>Server neu starten.</li>';
    echo '</ol>';
    echo '</body></html>';
    return;
}
